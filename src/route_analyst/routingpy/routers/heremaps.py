...
class HereMaps:
    ...
    def __init__(
        ...
        **client_kwargs
        ...
        if app_id is None and app_code is None and api_key is None:
            raise KeyError(
                "HERE Maps app_id and app_code, or api_key must be specified."
            )
    ...
    def directions(  # noqa: C901
        ...
        if avoid_seasonal_closures is not None:
            params["avoidSeasonalClosures"] = convert.convert_bool(
        ...
        if maneuver_attributes is not None:
            params["maneuverAttributes"] = convert.delimit_list(
                maneuver_attributes, ","
            )
        ...
        if generalization_tolerances is not None:
            params["generalizationTolerances"] = convert.delimit_list(
        ...
        if avoid_transport_types is not None:
            params["avoidTransportTypes"] = convert.delimit_list(
        ...
        if shipped_hazardous_goods is not None:
            params["shippedHazardousGoods"] = convert.delimit_list(
    ...
    def _parse_direction_json(response, alternatives):
        ...
        if alternatives is not None and alternatives > 1:
            ...
            return Direction(
                geometry=geometry, duration=duration, distance=distance, raw=response
            )
        ...
        for idx, isochrones in enumerate(response["response"]["isoline"]):
            ...
            geometries.append(
                Isochrone(
                    ...
                    center=list(
                        response["response"]["start"]["mappedPosition"].values()
                    ),
                )
            )
