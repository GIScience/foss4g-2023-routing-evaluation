...
class Graphhopper:
        ...
        if algorithm is not None:
            ...
            if algorithm == "alternative_route":
                if alternative_route_max_paths is not None:
                    params.append(
                        ("alternative_route.max_paths", alternative_route_max_paths)
                    )
        ...
        for index, polygon in enumerate(response[accessor]):
            isochrones.append(
                Isochrone(
                    geometry=[
                        l[:2]
                        for l in polygon["geometry"]["coordinates"][0]  # noqa: E741
                    ],  # takes in elevation for some reason
                    interval=int(
                        max_range * ((polygon["properties"]["bucket"] + 1) / buckets)
                    ),
                    center=center,
                )
            )
    ...
    def matrix(
        ...
        if sources is None and destinations is None:
            locations = (
                reversed([convert.format_float(f) for f in coord])
            ...
            try:
                ...
                raise IndexError(
                    "Parameter sources out of locations range at index {}.".format(idx)
                )
            except TypeError:
            ...
            try:
                ...
                raise IndexError(
                    "Parameter destinations out of locations range at index {}.".format(
                        idx
                    )
                )
            except TypeError:
